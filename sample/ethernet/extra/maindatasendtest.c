...
int main(void)
{
	...
	while (1)
	{
		...
		LogWrite(FromSample, LOG_NOTICE, "MAC ADDRESS IS: %u.%u.%u.%u.%u.%u",
			(unsigned)pARPFrame->ARP.HWAddressSender[0],
			(unsigned)pARPFrame->ARP.HWAddressSender[1],
			(unsigned)pARPFrame->ARP.HWAddressSender[2],
			(unsigned)pARPFrame->ARP.HWAddressSender[3],
			(unsigned)pARPFrame->ARP.HWAddressSender[4],
			(unsigned)pARPFrame->ARP.HWAddressSender[5]);		
		...
		if((unsigned)pARPFrame->ARP.ProtocolAddressSender[3] == 171)
		{
			LogWrite(FromSample, LOG_ERROR, "hiya PPV!!!!! :):P");
			 data[0] = 100;
			 data[1] = 200;
			 data[2] = 250;
			 data[3] = 255;
		...
		memcpy(pARPFrame->ARP.ProtocolAddressTarget, data, IP_ADDRESS_SIZE);
