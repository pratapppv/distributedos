...
int main (void)
{
	...
	while (1)
	{
		...
		LogWrite(FromSample, LOG_NOTICE,"SENDER ADDRESS    %u:%u:%u:%u:%u:%u",(unsigned)pARPFrame->ARP.HWAddressSender[0],(unsigned)pARPFrame->ARP.HWAddressSender[1],(unsigned)pARPFrame->ARP.HWAddressSender[2],(unsigned)pARPFrame->ARP.HWAddressSender[3],(unsigned)pARPFrame->ARP.HWAddressSender[4],(unsigned)pARPFrame->ARP.HWAddressSender[5]);
		// prepare reply packet
		*/
		u8 x[MAC_ADDRESS_SIZE] = { 0xC0, 0x18, 0x85, 0x58, 0xC9, 0x0F };
		...
		memcpy (pARPFrame->ARP.HWAddressTarget, x, MAC_ADDRESS_SIZE);
