...
int main(void)
{
	...
	if (!USPiInitialize())
	{
		LogWrite("ppv", LOG_ERROR, "Cannot initialize USPi");
	...
	{
		LogWrite("ppv", LOG_ERROR, "Ethernet device not found");
	...
	while (1)
	{
		...
		{
			LogWrite("ppv", LOG_ERROR, "USPiSendFrame failed");
			break;
		}

		LogWrite("ppv", LOG_ERROR, "SENT SUCCESSFULLY :P");
	}
return 0;
}